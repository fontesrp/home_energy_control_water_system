...
time_t processSyncMessage() {
	...
	while (Serial.available()) {
		...
		if (c == TIME_HEADER) {
			...
			for (i = 0; i < TIME_MSG_LEN && (c = Serial.read()); i++) {
			...
			if (i < TIME_MSG_LEN) {
				Serial.println("Error: entered value has length smaller then TIME_MSG_LEN");
				syncTime = 0;
			// } else {
			// 	return syncTime;
			}
			...
			Serial.println("Error: no TIME_HEADER");
			syncTime = 0;
		}
	}
